    ...
    gates: {
      ...
      items: {
        ...
        properties: {
          ...
          torBreiteM: { type: 'number', description: 'NUR setzen, wenn eine Tor-Breite AUSDRÜCKLICH genannt ist (z.B. "jedes Tor 4 m breit"). Sonst WEGLASSEN — niemals raten/defaulten!' },
          lueckeM: { type: 'number', description: 'LÜCKE (Zwischenraum) zwischen benachbarten Toren — NUR bei "Lücke", "zwischen den Toren", "X m dazwischen", "Abstand zum nächsten Tor". NICHT bei einem schlichten "Abstand N".' },
          spacingM: { type: 'number', description: 'Achsabstand Mitte-zu-Mitte. Ein schlichtes "Abstand N" / "Raster N" gehört HIERHER (NICHT nach lueckeM).' },
...
const SYSTEM = [
  ...
  'Tor-Breite NUR wenn ausdrücklich genannt ("jedes Tor 4 m breit") → gates[].torBreiteM; sonst',
  'torBreiteM WEGLASSEN (NIEMALS eine Breite raten). Ein schlichtes "Abstand N" / "Raster N" ist',
  'der Achsabstand → gates[].spacingM. NUR "Lücke", "zwischen den Toren", "X dazwischen" oder',
  '"Abstand zum nächsten Tor" ist die Lücke → gates[].lueckeM. Beispiele: "20 Tore Nord Abstand 6"',
  '→ {count:20,side:north,spacingM:6} (KEIN torBreiteM). "50 Tore, jedes 4 m breit, 3 m Abstand zum',
  'nächsten Tor" → {count:50,torBreiteM:4,lueckeM:3}.',
