...
function createSchmidHalle6(): {
  ...
  sektionenNord.forEach(s => {
    objects.push({ type: 'stellplatz', x: s.x, y: 4, width: s.w, height: 6, name: s.name + ' oben', color: '#334155' });
    objects.push({ type: 'stellplatz', x: s.x, y: 11, width: s.w, height: 6, name: s.name + ' unten', color: '#334155' });
  });
  ...
  sektionenSued.forEach(s => {
    objects.push({ type: 'stellplatz', x: s.x, y: 24, width: s.w, height: 6, name: s.name + ' oben', color: '#334155' });
    objects.push({ type: 'stellplatz', x: s.x, y: 31, width: s.w, height: 6, name: s.name + ' unten', color: '#334155' });
  });

  // SONDERBEREICHE
  objects.push({ type: 'bereich', x: 63, y: 5, width: 7, height: 12, name: 'BP 2', color: '#78716c' });
  objects.push({ type: 'bereich', x: 71, y: 5, width: 7, height: 12, name: 'BP 1', color: '#78716c' });
  objects.push({ type: 'bereich', x: 78, y: 25, width: 10, height: 8, name: 'ÜZ SE', color: '#a3a3a3' });
  objects.push({ type: 'stellplatz', x: 90, y: 25, width: 25, height: 12, name: 'Paletten', color: '#78716c' });
  objects.push({ type: 'bereich', x: 82, y: 34, width: 10, height: 4, name: 'BP 1 Süd', color: '#78716c' });
  objects.push({ type: 'bereich', x: 93, y: 34, width: 10, height: 4, name: 'BP 2 Süd', color: '#78716c' });
  objects.push({ type: 'buero', x: W - 6, y: 4, width: 5, height: 8, name: 'Büro', color: '#6366f1' });
  objects.push({ type: 'bereich', x: 142, y: 31, width: 7, height: 6, name: 'EX', color: '#dc2626' });

  // KETTE / FÖRDERBAND
  objects.push({ type: 'hindernis', x: 10, y: 19.5, width: 130, height: 3, name: 'Kette/Förderband', color: '#475569' });

  // ENTLADEZONEN
  objects.push({ type: 'entladebereich', x: 67, y: H - 4, width: 20, height: 2.5, name: 'EZ 1 (Tore 19-26)', color: '#4ade80' });
  objects.push({ type: 'entladebereich', x: 44, y: 1.5, width: 30, height: 2, name: 'EZ 2 (Tore 65-73)', color: '#4ade80' });
  objects.push({ type: 'entladebereich', x: 17, y: 1.5, width: 10, height: 2, name: 'EZ 3 (Tore 78-80)', color: '#4ade80' });

  // GÄNGE
  let gangId = 1;
  const gaenge: Gang[] = [];

  gaenge.push({ id: gangId++, name: 'Hauptgang Nord', points: [{ x: 0, y: 10.5 }, { x: W, y: 10.5 }], breite: 3.5, typ: 'hauptgang', farbe: '#22c55e' });
  gaenge.push({ id: gangId++, name: 'Hauptgang Mitte', points: [{ x: 0, y: 18.5 }, { x: W, y: 18.5 }], breite: 3, typ: 'hauptgang', farbe: '#22c55e' });
  gaenge.push({ id: gangId++, name: 'Hauptgang Mitte Süd', points: [{ x: 0, y: 23 }, { x: W, y: 23 }], breite: 3, typ: 'hauptgang', farbe: '#22c55e' });
  gaenge.push({ id: gangId++, name: 'Hauptgang Süd', points: [{ x: 0, y: 30.5 }, { x: W, y: 30.5 }], breite: 3.5, typ: 'hauptgang', farbe: '#22c55e' });

  const quergangX = [0.5, 29, 54, 63, 78, 99, 119, 141, W - 0.5];
  quergangX.forEach((x, i) => {
    gaenge.push({ id: gangId++, name: `Quergang ${i + 1}`, points: [{ x, y: 0 }, { x, y: H }], breite: 3, typ: 'quergang', farbe: '#22c55e' });
  });

  const zufahrtX = [14, 42, 66, 90, 110, 130];
  zufahrtX.forEach((x, i) => {
    gaenge.push({ id: gangId++, name: `Zufahrt ${i + 1}`, points: [{ x, y: 1.5 }, { x, y: 18 }], breite: 2.5, typ: 'regalgang', farbe: '#4ade80' });
...
/**
 * Lädt das Andreas Schmid Halle 6 Layout in den Store.
 * Resettet den aktuellen State vorher.
 */
export function loadSchmidLayout() {
