  ...
  for (let i = 0; i < passableGaenge.length; i++) {
    for (let j = i + 1; j < passableGaenge.length; j++) {
      ...
      intersections.forEach(intersection => {
        ...
        for (const gang of [passableGaenge[i], passableGaenge[j]]) {
          for (let k = 0; k < gang.points.length - 1; k++) {
            ...
            if (isPointOnSegment(intersection, segStart, segEnd, 0.5)) {
              const nodeA = nodes.find(n =>
                Math.abs(n.x - segStart.x) < 0.5 && Math.abs(n.y - segStart.y) < 0.5
              );
              const nodeB = nodes.find(n =>
                Math.abs(n.x - segEnd.x) < 0.5 && Math.abs(n.y - segEnd.y) < 0.5
              );
              if (nodeA && nodeA.id !== intersectionNode!.id) {
                const d = Math.sqrt((intersection.x - segStart.x) ** 2 + (intersection.y - segStart.y) ** 2);
                edges.push({ from: intersectionNode!.id, to: nodeA.id, distance: d, gangId: gang.id });
                edges.push({ from: nodeA.id, to: intersectionNode!.id, distance: d, gangId: gang.id });
              }
              if (nodeB && nodeB.id !== intersectionNode!.id) {
                const d = Math.sqrt((intersection.x - segEnd.x) ** 2 + (intersection.y - segEnd.y) ** 2);
                edges.push({ from: intersectionNode!.id, to: nodeB.id, distance: d, gangId: gang.id });
                edges.push({ from: nodeB.id, to: intersectionNode!.id, distance: d, gangId: gang.id });
              }
              break;
            }
          }
